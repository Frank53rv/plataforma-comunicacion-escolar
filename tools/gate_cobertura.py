...
UMBRAL = 70.0
p = os.path.join(RAIZ, 'coverage', '.last_run.json')
if not os.path.exists(p):
    R.aviso('sin coverage/.last_run.json · corré la suite con SimpleCov antes de integrar')
    R.cerrar()
try:
    v = json.load(open(p, encoding='utf-8'))['result']
    pct = float(v.get('line', v.get('covered_percent')))
except Exception as e:
    R.falla('coverage/.last_run.json ilegible: %s' % e); R.cerrar()
if pct < UMBRAL:
