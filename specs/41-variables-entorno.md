...
| RAILS_MASTER_KEY | Clave de descifrado de las credenciales cifradas del framework: el contenido de api/config/master.key, que no se versiona | RNF-03 · ISO/IEC 27001 |
...
| VITE_FCM_API_KEY, VITE_FCM_PROJECT_ID, VITE_FCM_SENDER_ID y VITE_FCM_APP_ID | Configuración del proyecto de notificaciones push en el cliente. Sin ellas el cliente no registra suscripción y los avisos se presentan dentro de la aplicación | RF-31 · RNF-11 |
| VITE_VAPID_PUBLIC_KEY | Clave pública de identificación del emisor ante el servicio de push web, configurada en Firebase y usada por el cliente | RF-31 · RNF-11 |

> Nota. El repositorio incluye un archivo de ejemplo con la totalidad de estas variables, su descripción y un valor de muestra que no es utilizable en ninguna instalación real; el archivo con los valores efectivos nunca se versiona. Las variables con prefijo VITE_ las lee el cliente al construirse; el servidor no usa un par de claves VAPID propio, porque con la versión HTTP v1 del servicio push ese par se configura en Firebase y lo usa el cliente. Es lo que permite que CP-RNF-19 se ejecute sobre un repositorio recién clonado sin editar ningún archivo manualmente.
